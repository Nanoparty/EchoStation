...
onready var numEnemies = 10
onready var numDeaths = 5
onready var numCoins = 100
...
func _on_Timer_timeout():
	if state == 0:
		enemiesText.text = "Enemies Defeated: 5"
		state = 1
	...
	if state == 1:
		...
		return
	if state == 2:
		coinsText.text = "Coins Collected: 110"
		state = 3
